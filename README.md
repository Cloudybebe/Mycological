...
lichen palette. A pixelated amoeboid sheet advances inward from the screen edges with broad lobes and a fine exploratory fringe, then resolves into a visibly crawling cellular transport network with irregular junction swellings, anchored heavy source channels, traveling expansion and contraction waves, moving wet highlights, and six-color lichen shading in one render pass. Wide channels blend continuously from warm brown-olive rims through mustard and gold bodies to pale wet centers, while only the finest channels near the one-pixel threshold use a stable lengthwise palette gradient instead of attempting an unreadable circular cross-section. The network grows over the first 12 seconds and retracts over the
